...
const VehiclePage = () => {
    ...
    const handleModalOk = async () => {
        try {
            ...
            // Xử lý dữ liệu văn bản: Đảm bảo số khung, số máy rỗng thì chuyển thành null (tránh lỗi trùng lặp Unique Constraint trong DB)
            ...
            // Đóng modal nhập liệu và hiển thị ngay màn hình Loading (tránh user bấm lưu nhiều lần)
            ...
            // ==============================================================================
            // BƯỚC 1: LƯU THÔNG TIN VĂN BẢN TRƯỚC (GỌI API TẠO/CẬP NHẬT XE)
            // ==============================================================================
            // Mục đích: Gửi biển số, màu sắc, loại xe... xuống DB trước để lấy ID của xe
            if (editingVehicle) {
                // Cập nhật thông tin chữ cho xe đã có sẵn
                vehicleId = editingVehicle.vehicleId || editingVehicle.id;
                await driverService.updateVehicle(vehicleId, payload);
            } else {
                // Tạo mới xe trên hệ thống, Backend sẽ trả về dữ liệu xe mới cùng với vehicleId
            ...
            // ==============================================================================
            // BƯỚC 2: UPLOAD HÌNH ẢNH XE LÊN AWS S3 (GỌI API UPLOAD ẢNH THEO TỪNG FILE)
            // ==============================================================================
            // Mục đích: Lấy vehicleId vừa tạo ở trên, lần lượt gọi API gửi từng tấm hình lên Server.
            // Server sẽ đẩy hình lên AWS S3, nhận URL về rồi mới lưu vào Database của xe.
            // Upload tuần tự (for...of) thay vì song song (Promise.all) để tránh lỗi ghi đè dữ liệu đồng thời trong DB.
            if (images && vehicleId) {
                ...
                for (const [key, file] of imageEntries) {
                    ...
                    try {
                        // Gọi API truyền vehicleId, file vật lý, và key (front, back, owner...) để Backend biết lưu vào cột nào
        ...
        <div>
            ...
            >
                ...
                <Form form={form} layout="vertical" style={{ marginTop: 10 }}>
                    <Row gutter={16}>
                        <Col span={12}>
                            <Form.Item name="licensePlate" label={isBicycle ? "License Plate (Auto-generated)" : "License Plate"} rules={[{ required: true }]}>
                                <Input size="large" disabled={isBicycle} placeholder={isBicycle ? "Auto-generated by system" : "e.g. 51F-123.45"} />
                            </Form.Item>
                        </Col>
                        <Col span={12}>
