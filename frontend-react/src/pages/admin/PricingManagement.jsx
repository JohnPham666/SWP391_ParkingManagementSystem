...
const PricingManagement = () => {
  ...
  const handleSave = async (values) => {
    try {
      if (editingRule) {
        await pricingApi.updatePricingRule(editingRule.pricingId, values);
  ...
  const columns = [
    { title: 'ID', dataIndex: 'pricingId', key: 'id' },
    { title: 'Vehicle Type', dataIndex: 'vehicleType', key: 'vehicleType', render: text => <strong>{text}</strong> },
    { title: 'Base Hourly Rate', dataIndex: 'baseHourlyRate', key: 'baseHourlyRate', render: (val) => `${val?.toLocaleString()} ₫` },
    { title: 'Max Daily Rate', dataIndex: 'maxDailyRate', key: 'maxDailyRate', render: (val) => `${val?.toLocaleString()} ₫` },
    {
      title: 'Action',
      key: 'action',
      render: (_, record) => (
        <>
          ...
          <Button type="link" danger icon={<DeleteOutlined />} onClick={() => handleDelete(record.pricingId)}>Delete</Button>
        </>
      ),
    },
  ];
  ...
  return (
    <Card 
      ...
      extra={<Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>Create Policy</Button>}
    >
      <Table columns={columns} dataSource={pricingRules} rowKey="pricingId" loading={loading} />
