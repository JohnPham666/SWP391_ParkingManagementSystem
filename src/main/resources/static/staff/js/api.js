...
const Api = {
    ...
    init() {
        const saved = localStorage.getItem('parking_staff_auth');
        if (saved) {
            try {
    ...
    saveAuth(data) {
        ...
        localStorage.setItem('parking_staff_auth', JSON.stringify(data));
    },

    clearAuth() {
        this.token = null;
        localStorage.removeItem('parking_staff_auth');
    },
